...
export default function ProductView({ product }: { product: Product }) {
  ...
  const { user, loginAsGuest } = useAuth();
    const naddToWishlist, removeFromWishlist, isInWishlist } = useWishlist();
            ...
            >
              <div className="bg-[#1c1c1e] border border-white/10 rounded-[40px] p-10 text-center shadow-[0_0_100px_rgba(0,0,0,0.5)]">
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <button 
                    onClick={() => { loginAsGuest(); setShowAuthGate(false); addToCart({ id: `${product.id}-${selectedVariant.color.toLowerCase().replace(/\s+/g, '-')}`, name: `${product.name} - ${selectedVariant.color}`, price: selectedVariant.price || product.basePrice, image: selectedVariant.images[0], color: selectedVariant.color, quantity: 1 }); } }
